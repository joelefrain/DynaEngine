...
class DynamicModelSpec:
    ...
    def from_mapping(cls, data: dict[str, Any]) -> "DynamicModelSpec":
        ...
        return cls(
            ...
            sigma_vertical_kpa=None if sigma_vertical is None else float(sigma_vertical),
...
def evaluate_dynamic_curve(spec: DynamicModelSpec | dict[str, Any]) -> DynamicCurveResult:
    model_spec = spec if isinstance(spec, DynamicModelSpec) else DynamicModelSpec.from_mapping(spec)
...
def _load_theoretical_curves() -> dict[str, Any]:
    ...
    return {
        ...
        "rojas": {
            "G_Gmax": {float(k): np.asarray(v, dtype=float) for k, v in raw["rojas"]["G_Gmax"].items()},
            "D": {float(k): np.asarray(v, dtype=float) for k, v in raw["rojas"]["D"].items()},
        },
        "seed": {
            "G_Gmax": {k: np.asarray(v, dtype=float) for k, v in raw["seed"]["G_Gmax"].items()},
...
def _interp_log(source_strain: np.ndarray, target_strain: np.ndarray, values: np.ndarray) -> np.ndarray:
    return np.interp(np.log10(target_strain), np.log10(source_strain), values)


def _darendeli_2001(p: dict[str, Any], sigma_vertical_kpa: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def _menq_2003(p: dict[str, Any], sigma_vertical_kpa: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    strain = DEFAULT_SHEAR_STRAIN
    sigma_norm = _mean_effective_stress_atm(p["k0"], sigma_vertical_kpa)
    gamma_ref = 0.12 * p["Cu"] ** -0.6 * sigma_norm ** (0.5 * p["Cu"] ** -0.15) / PERCENT_TO_NUMBER
...
def _rollins_2020(p: dict[str, Any], sigma_vertical_kpa: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    strain = DEFAULT_SHEAR_STRAIN
    sigma_m_kpa = _mean_effective_stress_atm(p["k0"], sigma_vertical_kpa) * ATM_PRESSURE_KPA
...
def _ishibashi_1993(p: dict[str, Any], sigma_vertical_kpa: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    strain = DEFAULT_SHEAR_STRAIN
    ip = p["IP"]
    if ip <= 15:
    ...
    damping = 100 * 0.333 * 0.5 * (1 + np.exp(exponent)) * (0.586 * ggmax**2 - 1.547 * ggmax + 1)
    return strain, ggmax, damping


def _wang_2021(p: dict[str, Any], sigma_vertical_kpa: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    strain = DEFAULT_SHEAR_STRAIN
    ...
    if group == "Clean sand and gravel group":
        ...
        if cf is None or wc is None:
            raise ValueError("Wang & Stokoe requiere CF y wc para Clean sand and gravel group")
        a = cf + 0.834
        b = 0.844 - 1.897 * cf
        ...
        if cf is None:
            raise ValueError("Wang & Stokoe requiere CF para Nonplastic silty sand group")
        ...
        d = 12.13
        c = 1.39 * p["e"] ** 0.27
        gamma_d = 0.0025 * (sigma_norm + 5.73 * p["e"] + 9.17 * cf) ** (1.47 - 0.52 * cf)
    elif group == "Clayey soil group":
        _require_wang_params(p, "e", "OCR")
        if cf is None or ip is None or wc is None:
            raise ValueError("Wang & Stokoe requiere CF, IP y wc para Clayey soil group")
        a = 0.896 + 0.412 * cf + 0.534 * ip
        b = 0.586 - 0.098 * p["e"] - 0.135 * cf
        gamma_mr = (0.02 * p["e"] + 0.004 * cf) * (sigma_norm + 0.42 * p["OCR"]) ** (0.447 - 0.27 * ip)
        cd = 4.86
        f_sigma = sigma_norm ** (-0.19)
        ...
        d = 21.7
        c = (1.91 * cf) ** (1.62 * ip)
        gamma_d = 0.11 * (0.12 * sigma_norm + 5.29 * wc - cf) ** (1.45 - ip + wc - 1.09 * cf)
    else:
...
def _rojas_2019(p: dict[str, Any], sigma_vertical_kpa: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    strain = DEFAULT_SHEAR_STRAIN
    sigma_m_kpa = _mean_effective_stress_atm(p["k0"], sigma_vertical_kpa) * ATM_PRESSURE_KPA
    ...
    ggmax = _interp_log(source_strain, strain, _DISCRETE_CURVES["rojas"]["G_Gmax"][sigma_ref])
    damping = _interp_log(source_strain, strain, _DISCRETE_CURVES["rojas"]["D"][sigma_ref])
    return strain, ggmax, damping
...
def _user_defined(curve_data: dict[str, Any] | None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    mask = np.isfinite(source_strain) & np.isfinite(source_ggmax) & np.isfinite(source_damping)
    ...
    strain = DEFAULT_SHEAR_STRAIN
    ggmax = np.interp(np.log10(strain), np.log10(source_strain), source_ggmax, left=source_ggmax[0], right=source_ggmax[-1])
    damping = np.interp(np.log10(strain), np.log10(source_strain), source_damping, left=source_damping[0], right=source_damping[-1])
    return strain, ggmax, damping
